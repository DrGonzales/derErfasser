...
{#if device}
    <div class="device-view">
        <button class="back" on:click={() => onBack && onBack()}>← Zurück</button>
        ...
        <dl>
            ...
            <div><dt>Seriennummer</dt><dd>{device.serialNumber}</dd></div>
            <div><dt>Schutzklasse</dt><dd>{device.protectionClass}</dd></div>
            <div><dt>Nennspannung</dt><dd>{device.ratedVoltage}</dd></div>
            <div><dt>Nennleistung</dt><dd>{device.ratedPower}</dd></div>
        </dl>
...
<style>
    ...
    th, td { border:1px solid #eee; padding:0.35rem }
</style>
